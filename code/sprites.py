...
class Sprite(pygame.sprite.Sprite):
    def __init__(self, pos, surf = pygame.Surface((TILE_SIZE, TILE_SIZE)), groups = None, z = Z_LAYERS['main']):
        ...
        self.image = surf
        self.rect = self.image.get_rect(topleft = pos) #frect doesnt exist?? frect is float rectangle so better hitbox
...
class Spike(Sprite):
    ...
    #------------------------------- Turn all parameters into attributes (again self is what turns these parameters useable for the class)
        self.center = pos
        self.radius = radius
        self.speed = speed
        ...
        #Use TRIG to create x values and y values
        y = self.center[1] + sin(radians(self.angle)) * self.radius #self.center moves the origin point
