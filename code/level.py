...
class Level: 
    ...
    #----------------------- groups --------------------------
        ...
        self.collision_sprites = pygame.sprite.Group() # This group should contain all collidable sprites
    ...
    #-----------------------------------SET UP THE TILES -----------
        ...
        for layer in ['BG', 'Terrain', 'FG', 'Platforms']: 
            for x, y, surface, in tmx_map.get_layer_by_name(layer).tiles(): #gets the coordintes of everythign
                ...
                Sprite((x * TILE_SIZE, y * TILE_SIZE), surface, groups, z)#Multiply by the tile size so that the objects are where it needs to be
    ...
    #--------------------------------------SET UP ALL THE OBJECTS (player is in obj) ------------------
        ...
        for obj in tmx_map.get_layer_by_name('Objects'):
            if obj.name == 'player':
                self.player = Player(
                    ...
                    jump_sound = audio_files['jump']) #The difference between terrain and the player is that the player ONLY HAS ACCESS to self.collision_sprites, but since 'Terrain', caontains (eslf.all_sprites, and self.collisionsrpites) in a touple, It will group that sprite into collisonsprites
            
            else: #everything but the player
    ...
    #----------------------------------------SET UP MOVING OBJECTS ---------------------------
        for obj in tmx_map.get_layer_by_name('Moving Objects'):
            ...
            if obj.name == 'spike':
                ...
                frames = level_frames[obj.name]
                groups = (self.all_sprites, self.semi_collision_sprites) if obj.properties['platform'] else (self.all_sprites, self.damage_sprites) #(self.all_sprites, self.damage_sprites)
                #print(frames)
            ...
            if obj.name == 'saw':
                if move_dir == 'x':
                    ...
                    left, right = int(start_pos[1]), int(end_pos[1])
                    for y in range(left, right, 20):
                        Sprite((x, y), level_frames['saw_chain'], self.all_sprites, z = Z_LAYERS['bg details'])
        ...
        for obj in tmx_map.get_layer_by_name('Enemies'):
            ...
            if obj.name == 'shell':
                Shell(pos=(obj.x, obj.y), 
                      ...
                      shell_sprite = self.shell_sprites)
    ...
    def create_pearl(self, pos, direction, shell_sprite):
        Pearl(pos, 
              (self.all_sprites, self.damage_sprites, self.pearl_sprites), 
              self.pearl_surf, 
              direction, 150, shell_sprite)
    ...
    def pearl_collision(self):
        ...
        #     shells_hit = pygame.sprite.spritecollide(pearl, self.collision_sprites, False)
        #     # for shell in shells_hit:
        #     #     #print(shell)
        #     #     pass
                
    def hit_collision(self):
        for sprite in self.damage_sprites:
            if isinstance(sprite, Tooth) and sprite.isalive:
    ...
    #------------------- Winner Winner Chicken Dinner Constraint---------------------------------------
    ...
    def run(self, delta_time):
        ...
        self.all_sprites.update(delta_time) #this calls all the update methods inside of it alloing for animation
